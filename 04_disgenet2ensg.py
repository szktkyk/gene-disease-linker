...
parser.add_argument("linked_data_Disease_term", help="linked life data disease term to search for")
parser.add_argument("score", type=float,help="minimum score to filter the genes")
...
def main():
    ...
    if not os.path.exists(f"./results"):
        os.mkdir(f"./results")
    ensg_list = disgenet.disgenet2list(args.linked_data_Disease_term,args.score,args.limit)
    print(ensg_list)
    df = pd.DataFrame(ensg_list)
    df.to_csv(f"./results/disgenet_genes_{args.score}.tsv",sep="\t",index=False)
